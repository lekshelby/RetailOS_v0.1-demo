...
async function showShiftReport(shiftId = state.shift?.id, reportActorId = state.user.id, sessionToken = null) {
  if (!shiftId) throw new Error('Open a shift to view its report');
...
async function closeShiftFromDialog() {
  ...
  $('#shift-close-dialog').classList.add('hidden'); await loadCurrentShift(); await showShiftReport(shiftId, manager.user.id, manager.sessionToken); showToast(closeResult.negativeStock?.length ? `Shift closed. ${closeResult.negativeStock.length} stock shortage${closeResult.negativeStock.length === 1 ? '' : 's'} need review.` : 'Shift closed. The shift report is ready to print.');
}
...
$('#printer-settings-form').addEventListener('submit', async (event) => { event.preventDefault(); try { const printerConnectionMethod = $('#printer-connection-method').value; const profile = await request('/management/company', { method: 'PUT', body: JSON.stringify({ companyId: state.config.company.id, actorId: state.user.id, receiptFooter: $('#profile-receipt-footer').value, receiptPaperWidthMm: Number($('#profile-paper-width').value), printerConnectionMethod }) }); state.config.company.printerConnectionMethod = profile.printerConnectionMethod; $('#management-panel').classList.add('hidden'); showToast('PC printer settings saved.'); } catch (error) { $('#management-message').textContent = error.message; } });
...
$('#receipt-history').addEventListener('click', async (event) => { const view = event.target.closest('[data-history-receipt]'); const print = event.target.closest('[data-print-history-receipt]'); try { if (view) { $('#receipt-lookup-no').value = view.dataset.historyReceipt; await showReceipt(view.dataset.historyReceipt); } else if (print) await printReceiptFromHistory(print.dataset.printHistoryReceipt); } catch (error) { $('#checkout-message').textContent = error.message; showAlert(error); } });

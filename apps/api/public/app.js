...
function showManagementTab(name) {
  const neededPermission = name === 'company' || name === 'staff' || name === 'bukku' ? 'company.manage' : name === 'printer' ? 'printer.manage' : name === 'products' ? 'catalog.manage' : name === 'contacts' ? 'contact.manage' : null;
  if (neededPermission && !hasPermission(neededPermission)) throw new Error('You do not have access to this section');
  ['settings', 'company', 'printer', 'bukku', 'staff', 'products', 'contacts'].forEach((section) => $(`#management-${section}`).classList.toggle('hidden', section !== name));
  ...
  const copy = { settings: ['Settings', 'Company, staff, Bukku and printer settings.'], company: ['Company details', 'Business details used on every receipt.'], printer: ['Printer settings', 'Receipt paper and footer message.'], bukku: ['Bukku daily invoice', 'Review one consolidated daily invoice for a closed shift before any posting is enabled.'], staff: ['Staff accounts', 'Create real cashier and manager accounts, then retire demo accounts.'], products: ['Products', 'Search your product catalogue. Use + to add a new local product.'], contacts: ['Contacts', 'Search your customer and contact list. Use + to add a new local contact.'] };
...
async function loadCompanyProfile() {
  ...
  $('#bukku-daily-enabled').checked = Boolean(profile.bukkuDailyInvoiceEnabled); $('#bukku-daily-contact-id').value = profile.bukkuDailyInvoiceContactId || ''; $('#bukku-daily-location-id').value = profile.bukkuDailyInvoiceLocationId || ''; $('#bukku-daily-revenue-account-id').value = profile.bukkuDailyInvoiceRevenueAccountId || ''; $('#bukku-daily-tax-code-id').value = profile.bukkuDailyInvoiceTaxCodeId || '';
  const paymentAccounts = profile.bukkuDailyInvoicePaymentAccounts || {}; $('#bukku-payment-cash').value = paymentAccounts.CASH || ''; $('#bukku-payment-card').value = paymentAccounts.CARD || ''; $('#bukku-payment-duitnow').value = paymentAccounts.DUITNOW || ''; $('#bukku-payment-bank-transfer').value = paymentAccounts.BANK_TRANSFER || ''; $('#bukku-payment-store-credit').value = paymentAccounts.STORE_CREDIT || ''; $('#bukku-payment-other').value = paymentAccounts.OTHER || '';

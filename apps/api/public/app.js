...
async function showShiftReport(shiftId = state.shift?.id, reportActorId = state.user.id, sessionToken = null) {
  ...
  $('#printable-receipt').innerHTML = `<div class="receipt-head"><h3>Shift report</h3><p>${escapeHtml(report.shift.location)} · ${escapeHtml(report.shift.register)}</p><p>Cashier: ${escapeHtml(report.shift.cashier)}</p><p>Opened: ${new Date(report.shift.openedAt).toLocaleString('en-MY')}</p>${report.shift.closedAt ? `<p>Closed: ${new Date(report.shift.closedAt).toLocaleString('en-MY')}</p>` : ''}</div><div class="receipt-lines"><p>Sales ${report.summary.salesCount} · Gross ${money(report.summary.grossSales)}</p><p>Discounts ${money(report.summary.discountTotal)}</p><p>Cash sales ${money(report.summary.cashSales)} · Cash refunds ${money(report.summary.cashRefunds)}</p><p>Cash in ${money(report.summary.cashIn)} · Cash out ${money(report.summary.cashOut)}</p><p>Opening float ${money(report.summary.openingFloat)}</p></div><div class="receipt-summary"><div><span>Expected cash</span><strong>${money(report.summary.expectedCash)}</strong></div>${report.summary.variance !== undefined ? `<div><span>Variance</span><strong>${money(report.summary.variance)}</strong></div>` : ''}</div>${stockFollowUp}<div class="receipt-payment"><h3>Payments</h3><ul>${payments}</ul><h3>Returns</h3><ul>${refunds}</ul></div><div class="receipt-actions"><button type="button" class="primary" data-print-shift-report="${escapeHtml(report.shift.id)}">Print shift report</button></div>`;

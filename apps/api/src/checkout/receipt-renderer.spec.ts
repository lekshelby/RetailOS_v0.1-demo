...
const sale = (width: number, template = 'STANDARD') => ({ status: 'COMPLETED', receiptNo: 'TEST-001', completedAt: new Date('2026-09-01T10:00:00Z'), subtotal: decimal(23.5), discountTotal: decimal(1.5), grandTotal: decimal(22), eInvoiceRequestToken: 'qr-test-token', company: { name: '测试商店', legalName: 'Test Store 测试商店 Sdn Bhd', brnNew: 'BRN1', registrationNo: null, brnOld: null, tin: 'TIN1', address: 'Long address 测试街道', officePhone: '03-123', phone: '012-345', email: 'x@example.test', receiptFooter: '谢谢 / Thank you', receiptPaperWidthMm: width, receiptTemplate: template, receiptDividerStyle: 'DASHED', receiptShowSku: true, customerEInvoiceRequestsEnabled: true }, location: { name: 'Main' }, register: { name: 'R1' }, cashier: { name: 'Cashier' }, items: [{ description: 'A very long 商品 item name that must wrap only inside the description column', product: { sku: 'SKU-1' }, quantity: decimal(2), uom: { name: 'Each' }, unitPrice: decimal(11.75), lineDiscount: decimal(1.5), lineTotal: decimal(22) }], payments: [{ method: 'CASH', amount: decimal(20), tenderedAmount: decimal(20), changeAmount: decimal(8) }, { method: 'DUITNOW', amount: decimal(10), tenderedAmount: decimal(10), changeAmount: decimal(0) }] });

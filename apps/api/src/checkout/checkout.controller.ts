...
export class CheckoutController {
  ...
  @Get('printer/health') printerHealth(@Query() input: MarkReceiptPrintedDto) { return this.checkoutService.printerHealth(input); }
  ...
  @Get('receipt/:receiptNo/document') document(@Param('receiptNo') receiptNo: string, @Query('companyId') companyId: string) { return this.checkoutService.receiptDocument(receiptNo, companyId); }
  ...
  @Get(':id/receipt') receipt(@Param('id') id: string, @Query('companyId') companyId: string) { return this.db.sale.findFirstOrThrow({ where: { id, companyId }, include: { company: true, location: true, register: true, cashier: { select: { name: true } }, customer: true, items: { include: { uom: true } }, payments: true, discounts: true, returns: { where: { status: 'COMPLETED' }, include: { payments: true }, orderBy: { createdAt: 'desc' } } } }); }
}

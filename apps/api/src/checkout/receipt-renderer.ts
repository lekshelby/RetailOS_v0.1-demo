...
export function canonicalReceipt(sale: SaleForReceipt, status: ReceiptStatus = sale.status === 'VOIDED' ? 'VOID' : 'ORIGINAL'): CanonicalReceipt {
  ...
  return {
    version: 2, status, widthMm: company.receiptPaperWidthMm, template, showLogo: Boolean(sale.company.receiptShowLogo), showSku: Boolean(company.receiptShowSku), divider: company.receiptDividerStyle === 'DOUBLE' ? '=' : company.receiptDividerStyle === 'DOT' ? '.' : '-',
